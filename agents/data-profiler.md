...
| 조건 | 추천 | 근거 필수 |
|------|------|----------|
| 품질 점수 전항목 "양호" | data-clean 선택사항, stat으로 진행 가능 | 지표 목록 명시 |
| 품질 점수 "주의" 항목 존재 | data-clean 추천 (대상 컬럼 명시) | 영향 컬럼 목록 명시 |
| 품질 점수 "심각" 항목 존재 | data-clean 필수 (분석 전 처리 선행) | 심각도 이유 명시 |
...
예시:
> **다음 단계: harnessda:data-clean** — Income, Family, CCAvg 3개 컬럼 결측률 12% (caution). 타겟 변수(Personal Loan)와의 관계 분석 전 대체 처리 필요. 해당 컬럼이 분석 대상 외라면 stat으로 바로 진행 가능.
