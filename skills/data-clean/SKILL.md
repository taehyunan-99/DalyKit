...
harnessda:clean              ← 현재 디렉토리의 CSV 자동 탐색
harnessda:clean data.csv     ← 특정 파일 지정
harnessda:clean update       ← 기존 clean_pipeline.py 재실행 + JSON + report 갱신
```
...
project/
├── 01_eda.ipynb            ← EDA 단계
├── 02_preprocessing.ipynb  ← (선택) 결과 확인용 노트북
├── data/                   ← 원본 데이터
...
- `docs/eda_report.md`가 있으면 Read로 읽고, 결측값·이상치·타입 이슈를 파악하여 전처리 전략에 반영
- 없으면 데이터 파일을 직접 프로파일링 (dtypes, 결측값, 중복 수 확인)
...
- Write 도구로 `clean_pipeline.py` 생성
- Bash 도구로 `python clean_pipeline.py` 실행 → `clean_results.json` + `cleaned/` 저장

### 4단계: 전처리 보고서 자동 생성

스크립트 실행 완료 후 **자동으로** 이어서 실행한다.

1. `clean_results.json` 읽고 분석
2. 전처리 보고서를 작성하여 `docs/preprocessing_report.md`에 저장
...
1. `clean_pipeline.py` 존재 여부 확인 (Glob)
...
4. **단계별 JSON 저장**: 처리 결과를 `clean_results.json`에 구조화하여 저장
5. **이상치**: 자동 제거하지 않고 탐지만 → 사용자에게 판단 요청
6. **저장 경로**: `data/cleaned/파일명_cleaned.csv`
