...
`~/.claude/templates/`에서 템플릿을 읽어 `dalykit/config/`에 저장한다.

1. `~/.claude/templates/DOMAIN_TEMPLATE.md`를 Read로 읽는다
2. 내용을 `dalykit/config/domain.md`에 Write로 저장
3. `~/.claude/templates/REPORT_CONFIG_TEMPLATE.md`를 Read로 읽는다
4. 내용을 `dalykit/config/report_config.md`에 Write로 저장
...
2. hook 스크립트 복사:
    ...
    # Windows (PowerShell)
    Copy-Item "$env:USERPROFILE\.claude\hooks\guard_write.py" ".claude\hooks\guard_write.py"
    Copy-Item "$env:USERPROFILE\.claude\hooks\guard_read.py" ".claude\hooks\guard_read.py"
    ```
            ...
            "PreToolUse": [
                ...
                },
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "command": "python3 .claude/hooks/guard_read.py"
                        }
                    ]
                }
            ]
