...
2. `.claude/hooks/guard_write.py`와 `.claude/hooks/guard_read.py`를 아래 래퍼 코드로 각각 Write 생성 (marketplace 최신 파일을 동적 실행):

    **guard_write.py 래퍼:**
    ```python
    #!/usr/bin/env python3
    """DalyKit Write Guard — marketplace 래퍼. 플러그인 업데이트 시 자동으로 최신 로직 실행."""
    import subprocess, sys
    from pathlib import Path

    target = Path.home() / ".claude/plugins/marketplaces/taehyunan/hooks/guard_write.py"
    if target.exists():
        data = sys.stdin.buffer.read()
        result = subprocess.run([sys.executable, str(target)], input=data)
    ...
    **guard_read.py 래퍼:**
    ```python
    #!/usr/bin/env python3
    """DalyKit Read Guard — marketplace 래퍼. 플러그인 업데이트 시 자동으로 최신 로직 실행."""
    import subprocess, sys
    from pathlib import Path

    target = Path.home() / ".claude/plugins/marketplaces/taehyunan/hooks/guard_read.py"
    if target.exists():
        data = sys.stdin.buffer.read()
        result = subprocess.run([sys.executable, str(target)], input=data)
        sys.exit(result.returncode)
